...
3. **Seguridad Teórico-Informacional (ITS):** En modo `EntropySource.crypto()` / `hardware()`, la matriz de Toeplitz para la amplificación de privacidad se genera mediante entropía criptográfica directa (`os.urandom`), garantizando la cota de secreto de la Leftover Hash Lemma ($2^{-\ell}$) sin depender de un PRNG de estado acotado.
